      ...
      try {
        ...
        const [serverVideos, serverNotes] = await Promise.all([
          actor.getVideoBookmarks().catch((err) => {
            console.error(
              "[Bookmarks] Failed to fetch video bookmarks from backend:",
              err,
            );
            return [];
          }),
          actor.getNoteBookmarks().catch((err) => {
            console.error(
              "[Bookmarks] Failed to fetch note bookmarks from backend:",
              err,
            );
            return [];
          }),
        ...
        try {
          ...
          console.error(
            "[Bookmarks] Failed to parse local video bookmarks during sync:",
            e,
          );
        }
        ...
        try {
          ...
          console.error(
            "[Bookmarks] Failed to parse local note bookmarks during sync:",
            e,
          );
        }
        ...
        for (const id of serverVideos as string[]) {
        ...
        for (const id of serverNotes as string[]) {
    ...
    if (exists) {
      ...
      if (actor && identity) {
        actor.unbookmarkVideo(id).catch((err) => {
          console.error(
            "[Bookmarks] Failed to unbookmark video on backend:",
            err,
          );
        });
      }
      ...
      if (actor && identity) {
        actor.bookmarkVideo(id).catch((err) => {
          console.error(
            "[Bookmarks] Failed to bookmark video on backend:",
            err,
          );
        });
      }
    ...
    if (exists) {
      ...
      if (actor && identity) {
        actor.unbookmarkNote(id).catch((err) => {
          console.error(
            "[Bookmarks] Failed to unbookmark note on backend:",
            err,
          );
        });
      }
